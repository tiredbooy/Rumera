...
func scanHeroSlide(row pgx.Row, s *HeroSlide) error {
	return row.Scan(
...
func (r *repository) queryMany(ctx context.Context, query string, args ...any) ([]*HeroSlide, error) {
...
func (r *repository) GetActive(ctx context.Context) ([]*HeroSlide, error) {
...
func (r *repository) GetAll(ctx context.Context) ([]*HeroSlide, error) {
	query := `SELECT ` + heroSlideColumns + ` FROM hero_slides ORDER BY sort_order ASC, id ASC`
	return r.queryMany(ctx, query)
}

func (r *repository) GetByID(ctx context.Context, id int64) (*HeroSlide, error) {
...
func (r *repository) Create(ctx context.Context, req *HeroSlideReq) (*HeroSlide, error) {
...
func (r *repository) Update(ctx context.Context, id int64, req *HeroSlideUpdateReq) (*HeroSlide, error) {
...
func (r *repository) Reorder(ctx context.Context, ids []int64) error {
	...
	if err != nil {
		return fmt.Errorf("repository.Reorder begin: %w", err)
	}
	...
	if _, err := tx.Exec(ctx, `LOCK TABLE hero_slides IN SHARE ROW EXCLUSIVE MODE`); err != nil {
		return fmt.Errorf("repository.Reorder lock: %w", err)
	}
	var matches bool
	if err := tx.QueryRow(ctx, `
		...
		return fmt.Errorf("repository.Reorder validate: %w", err)
	}
	if !matches {
		return models.ErrInvalidState
	}
	...
	if err != nil {
		return fmt.Errorf("repository.Reorder update: %w", err)
	}
	if tag.RowsAffected() != int64(len(ids)) {
		return models.ErrInvalidState
	}
	if err := tx.Commit(ctx); err != nil {
		return fmt.Errorf("repository.Reorder commit: %w", err)
	}
	return nil
}
