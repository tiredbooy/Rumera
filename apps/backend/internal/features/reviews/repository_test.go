...
func TestGetMineSQLHasLimit100(t *testing.T) {
	fn := repoMethodSource(t, "func (r *repository) GetMine")
	if !strings.Contains(fn, "LIMIT 100") {
		t.Fatal("GetMine missing LIMIT 100")
	}
}

func TestGetPendingSQLHasLimit100(t *testing.T) {
	fn := repoMethodSource(t, "func (r *repository) GetPending")
	if !strings.Contains(fn, "LIMIT 100") {
		t.Fatal("GetPending missing LIMIT 100")
	}
}
