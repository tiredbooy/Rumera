...
type Repository interface {
	...
	Adjust(ctx context.Context, tx pgx.Tx, variantID int64, req AdjustStockReq, orderID *int64) error
...
func (r *inventoryRepository) GetByVariantID(ctx context.Context, variantID int64) (*Inventory, error) {
	...
	inv, err := pgx.CollectOneRow(rows, pgx.RowToStructByName[Inventory])
	if err != nil {
...
func (r *inventoryRepository) GetAll(ctx context.Context, f InventoryFilter) ([]*Inventory, int64, error) {
	where := []string{"1=1"}
	args := pgx.NamedArgs{}
	...
	q := fmt.Sprintf(`
		...
		SELECT p.id, p.product_variant_id, p.product_id, p.product_title,
		       p.sku, p.category_title, p.unit_price, p.weight, p.stock_on_hand,
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&sku, &categoryTitle, &unitPrice, &weight, &stockOnHand, &committedStock,
...
func (r *inventoryRepository) GetLowStock(ctx context.Context) ([]*Inventory, error) {
	...
	invs, err := pgx.CollectRows(rows, pgx.RowToStructByName[Inventory])
	if err != nil {
		return nil, fmt.Errorf("inventoryRepository.GetLowStock scan: %w", err)
	}

	result := make([]*Inventory, len(invs))
	for i := range invs {
		result[i] = &invs[i]
	}
	return result, nil
...
func (r *inventoryRepository) Adjust(ctx context.Context, tx pgx.Tx, variantID int64, req AdjustStockReq, orderID *int64) error {
...
func (r *inventoryRepository) Reserve(ctx context.Context, tx pgx.Tx, variantID int64, quantity int, orderID int64) error {
	...
	return recordMovement(ctx, tx, variantID, -quantity, MovementTypeReservation, &orderID, &note)
}
...
func (r *inventoryRepository) Release(ctx context.Context, tx pgx.Tx, variantID int64, quantity int, orderID int64) error {
	...
	return recordMovement(ctx, tx, variantID, quantity, MovementTypeRelease, &orderID, &note)
}
...
func (r *inventoryRepository) Deduct(ctx context.Context, tx pgx.Tx, variantID int64, quantity int, orderID int64) error {
	...
	return recordMovement(ctx, tx, variantID, -quantity, MovementTypePurchase, &orderID, &note)
}

func (r *inventoryRepository) UpdateReorder(ctx context.Context, variantID int64, req UpdateReorderReq) (*Inventory, error) {
	sets := []string{}
...
func recordMovement(ctx context.Context, tx pgx.Tx, variantID int64, quantity int, movType MovementType, orderID *int64, note *string) error {
	const q = `
