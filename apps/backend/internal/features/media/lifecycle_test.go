...
func TestMediaLifecycleCleanupRemovesOriginalAndDerivatives(t *testing.T) {
	...
	service := NewLifecycleService(store, cache, repo, zap.NewNop())
...
func TestMediaLifecycleDerivativeCleanupFailureIsRetryable(t *testing.T) {
	...
	service := NewLifecycleService(store, cache, &mediaLifecycleRepositoryStub{}, zap.NewNop())
	const key = "products/4-bottle/gallery-image.webp"
...
func TestMediaReconciliationDryRunAndApply(t *testing.T) {
	...
	dryRun, err := service.Reconcile(ctx, ReconcileOptions{MinimumAge: 24 * time.Hour, Now: now})
	if err != nil {
	...
	applied, err := service.Reconcile(ctx, ReconcileOptions{Apply: true, MinimumAge: 24 * time.Hour, Now: now})
	if err != nil {
...
func TestMediaReconciliationRechecksCandidateBeforeDelete(t *testing.T) {
	...
	service := NewLifecycleService(store, lifecycleStorage(t), repo, zap.NewNop())

	report, err := service.Reconcile(ctx, ReconcileOptions{
		Apply: true,
...
func TestMediaReconciliationReusesReviewedCutoff(t *testing.T) {
	...
	report, err := service.Reconcile(context.Background(), ReconcileOptions{
	...
	if _, err := service.Reconcile(context.Background(), ReconcileOptions{
...
func (r *mediaLifecycleRepositoryStub) LockMediaKeys(context.Context, []string) (KeyLock, error) {
	return mediaReconciliationLockStub{}, nil
}

func (r *mediaLifecycleRepositoryStub) TryReconciliationLock(context.Context) (ReconciliationLock, bool, error) {
