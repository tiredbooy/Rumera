...
// LifecycleService safely removes detached local originals and every
...
func NewLifecycleService(
	...
	return &LifecycleService{store: store, cache: cache, repo: repo, log: log}
}

// ProductKeys captures local images before a product cascade removes their rows.
func (s *LifecycleService) ProductKeys(ctx context.Context, productID int64) ([]string, error) {
	if s == nil || s.repo == nil {
		return nil, nil
	}
...
func (s *LifecycleService) VariantKeys(ctx context.Context, variantID int64) ([]string, error) {
	if s == nil || s.repo == nil {
		return nil, nil
	}
...
func (s *LifecycleService) CleanupURLs(ctx context.Context, values ...*string) {
...
func (s *LifecycleService) CleanupKeys(ctx context.Context, keys ...string) {
	if s == nil {
		return
	}
...
func (s *LifecycleService) ReleaseStandalone(ctx context.Context, key string) error {
...
func (s *LifecycleService) LockMediaKeys(ctx context.Context, keys ...string) (KeyLock, error) {
	if s == nil || s.repo == nil {
...
func (s *LifecycleService) deleteIfUnreferenced(ctx context.Context, key string) (bool, error) {
	if s == nil || s.repo == nil {
...
func (s *LifecycleService) Reconcile(ctx context.Context, options ReconcileOptions) (report ReconcileReport, resultErr error) {
	...
	report = ReconcileReport{
		...
		StartedAt: now,
		Cutoff:    cutoff,
		Objects:   make([]ReconcileObject, 0),
