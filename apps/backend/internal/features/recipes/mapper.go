...
func ToRecipeResponses(rs []*Recipe) []RecipeResponse {
	out := make([]RecipeResponse, len(rs))
	for i, r := range rs {
...
func ToRecipeListItem(r *Recipe) RecipeListItem {
...
func ToRecipeListItems(rs []*Recipe) []RecipeListItem {
	out := make([]RecipeListItem, len(rs))
	for i, r := range rs {
...
func ToRecipeAdminListItem(r *Recipe) RecipeAdminListItem {
	return RecipeAdminListItem{
...
func ToRecipeAdminListItems(rs []*Recipe) []RecipeAdminListItem {
	out := make([]RecipeAdminListItem, len(rs))
	for i, r := range rs {
...
func ToRecipeIngredientResponse(i *RecipeIngredient) RecipeIngredientResponse {
...
func ToRecipeIngredientResponses(ingredients []*RecipeIngredient) []RecipeIngredientResponse {
	out := make([]RecipeIngredientResponse, len(ingredients))
...
func ToShoppableProductResponses(products []*ShoppableProduct) []ShoppableProductResponse {
	out := make([]ShoppableProductResponse, len(products))
	for i, p := range products {
		out[i] = ShoppableProductResponse{
...
func ToRecipeTagInfos(tags []*RecipeTagInfo) []RecipeTagInfo {
	out := make([]RecipeTagInfo, len(tags))
	for i, t := range tags {
		out[i] = *t
	}
	return out
...
func BuildRecipeJSONLD(r *Recipe, ingredients []*RecipeIngredient) map[string]any {
	ld := map[string]any{
