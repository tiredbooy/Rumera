...
type Repository interface {
	...
	FindPendingByReferee(ctx context.Context, refereeID int64) (*Referral, error)
...
func (r *referralRepository) FindPendingByReferee(ctx context.Context, refereeID int64) (*Referral, error) {
	const q = `
	...
	rows, err := r.db.Query(ctx, q, refereeID, ReferralStatusPending)
	if err != nil {
		return nil, fmt.Errorf("referralRepository.FindPendingByReferee: %w", err)
	}
	ref, err := pgx.CollectOneRow(rows, pgx.RowToStructByName[Referral])
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
...
func (r *referralRepository) Complete(ctx context.Context, id int64) error {
	...
	if _, err := r.db.Exec(ctx, q, id, ReferralStatusCompleted, ReferralStatusPending); err != nil {
