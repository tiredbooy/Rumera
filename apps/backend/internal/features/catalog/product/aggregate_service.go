...
func (s *Service) resolveAggregateImages(
	ctx context.Context,
	req *SaveProductAggregateReq,
) error {
	fields := make(map[string][]string)
	for i := range req.Images {
		...
		if image.ImageURL != nil {
			normalized, err := media.NormalizeExternalImageURL(*image.ImageURL)
			if err != nil {
...
func cloneProductAggregateRequest(req SaveProductAggregateReq) SaveProductAggregateReq {
	req.MetaTags = append([]string(nil), req.MetaTags...)
	req.TagIDs = append([]int64(nil), req.TagIDs...)
	req.Variants = append([]SaveProductVariantReq(nil), req.Variants...)
	for i := range req.Variants {
		req.Variants[i].OptionValueIDs = append([]int64(nil), req.Variants[i].OptionValueIDs...)
	}
	req.Images = append([]SaveProductImageReq(nil), req.Images...)
	return req
}

func aggregatePreparedStorageKeys(images []SaveProductImageReq) []string {
...
func hashProductAggregateRequest(productID int64, req SaveProductAggregateReq) (string, error) {
	payload, err := json.Marshal(struct {
		ProductID int64                   `json:"product_id"`
		Request   SaveProductAggregateReq `json:"request"`
