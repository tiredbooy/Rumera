...
type tagServiceRepo struct {
	Repository
	createdReq     CreateTagReq
	updatedReq     UpdateTagReq
	updatedID      int64
...
func (r *tagServiceRepo) Create(_ context.Context, req CreateTagReq) (*Tag, error) {
	r.createdReq = req
	if r.createErr != nil {
		return nil, r.createErr
	}
	return &Tag{ID: 1, Title: req.Title, Slug: req.Slug, Description: req.Description}, nil
}

func (r *tagServiceRepo) GetByID(_ context.Context, id int64) (*Tag, error) {
	if r.getErr != nil {
		return nil, r.getErr
	}
	if r.current != nil {
	...
	return &Tag{ID: id, Title: "old title", Slug: "old-slug"}, nil
}

func (r *tagServiceRepo) Update(_ context.Context, id int64, req UpdateTagReq) (*Tag, error) {
	r.updateCalls++
	r.updatedID = id
	r.updatedReq = req
	if r.updateErr != nil {
...
func TestTagServiceCreateDerivesUnicodeSlug(t *testing.T) {
	repo := &tagServiceRepo{}
	tag, err := NewService(repo).Create(context.Background(), CreateTagReq{Title: "  نوشیدنی ویژه  "})
	if err != nil {
...
func TestTagServiceCreateNormalizesAndChecksSlugConflict(t *testing.T) {
	...
	_, err := NewService(repo).Create(context.Background(), CreateTagReq{
		Title: "Summer",
		Slug:  " Summer  Sale ",
	})
...
func TestTagServiceCreateMapsDatabaseConflict(t *testing.T) {
	...
	_, err := NewService(repo).Create(context.Background(), CreateTagReq{Title: "Gift", Slug: "gift"})
	if !errors.Is(err, apperr.ErrConflict) {
		t.Fatalf("err = %v; want ErrConflict", err)
	}
}
...
func TestTagServiceUpdateExcludesCurrentRowAndClearsDescription(t *testing.T) {
	...
	title := "  هدیه ویژه "
	slug := " Gift  Set "
	req := UpdateTagReq{Title: &title, Slug: &slug}
	req.Description.Set = true

	tag, err := NewService(repo).Update(context.Background(), 9, req)
	if err != nil {
...
func TestTagServiceUpdateAndDeleteMapNotFound(t *testing.T) {
	...
	if _, err := svc.Update(context.Background(), 10, UpdateTagReq{Title: &title}); !errors.Is(err, apperr.ErrNotFound) {
...
func TestTagServiceUpdateChecksExistenceBeforeConflicts(t *testing.T) {
	...
	_, err := NewService(repo).Update(
		context.Background(),
		99,
		UpdateTagReq{Title: &title},
	)
...
func TestTagServiceUpdateSupportsDescriptionOnlyAndNoOpPatches(t *testing.T) {
	...
	repo := &tagServiceRepo{current: &Tag{ID: 4, Title: "Gift", Slug: "gift"}}
	req := UpdateTagReq{}
	req.Description.Set = true
	req.Description.Value = &description

	tag, err := NewService(repo).Update(context.Background(), 4, req)
	if err != nil {
	...
	repo.updateCalls = 0
	tag, err = NewService(repo).Update(context.Background(), 4, UpdateTagReq{})
