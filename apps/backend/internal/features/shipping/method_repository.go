...
// MethodRepository is the persistence port for shipping methods.
type MethodRepository interface {
	Create(ctx context.Context, zoneID int64, req CreateShippingMethodReq) (*ShippingMethod, error)
	GetByID(ctx context.Context, id int64) (*ShippingMethod, error)
	GetByZoneID(ctx context.Context, zoneID int64, filter ShippingMethodFilter) ([]*ShippingMethod, int64, error)
	...
	GetAvailable(ctx context.Context, zoneID int64, weightKg float64) ([]*ShippingMethod, error)

	Update(ctx context.Context, id int64, req UpdateShippingMethodReq) (*ShippingMethod, error)
...
// NewMethodRepository constructs a Postgres method repository.
func NewMethodRepository(db *pgxpool.Pool) MethodRepository {
	return &methodRepository{db: db}
}

func (r *methodRepository) Create(ctx context.Context, zoneID int64, req CreateShippingMethodReq) (*ShippingMethod, error) {
	isActive := true
	if req.IsActive != nil {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("shipping.methodRepository.Create: %w", err)
	}
	return method, nil
}

func (r *methodRepository) GetByID(ctx context.Context, id int64) (*ShippingMethod, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("shipping.methodRepository.GetByID: %w", err)
	}
	return method, nil
}

func (r *methodRepository) GetByZoneID(ctx context.Context, zoneID int64, f ShippingMethodFilter) ([]*ShippingMethod, int64, error) {
	...
	if err := r.db.QueryRow(ctx, countQ, args).Scan(&total); err != nil {
		return nil, 0, fmt.Errorf("shipping.methodRepository.GetByZoneID count: %w", err)
	}
	...
	if err != nil {
		return nil, 0, fmt.Errorf("shipping.methodRepository.GetByZoneID: %w", err)
	}
	defer rows.Close()

	methods := make([]*ShippingMethod, 0, f.Limit)

	for rows.Next() {
		method, err := scanShippingMethod(rows)
		if err != nil {
			return nil, 0, fmt.Errorf("shipping.methodRepository.GetByZoneID scan: %w", err)
		}
		methods = append(methods, method)
	}
	if err := rows.Err(); err != nil {
		return nil, 0, fmt.Errorf("shipping.methodRepository.GetByZoneID rows: %w", err)
	}

	return methods, total, nil
}

// GetAvailable returns active methods for a zone that accept the order weight.
// NULL max_weight_kg means no weight limit — those always qualify.
func (r *methodRepository) GetAvailable(ctx context.Context, zoneID int64, weightKg float64) ([]*ShippingMethod, error) {
	const q = `
	...
	if err != nil {
		return nil, fmt.Errorf("shipping.methodRepository.GetAvailable: %w", err)
	}
	defer rows.Close()

	result := make([]*ShippingMethod, 0)
	for rows.Next() {
		method, err := scanShippingMethod(rows)
		if err != nil {
			return nil, fmt.Errorf("shipping.methodRepository.GetAvailable scan: %w", err)
		}
		result = append(result, method)
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("shipping.methodRepository.GetAvailable rows: %w", err)
	}
	return result, nil
}

func (r *methodRepository) Update(ctx context.Context, id int64, req UpdateShippingMethodReq) (*ShippingMethod, error) {
	sets := []string{}
	...
	if err != nil {
		...
		return nil, fmt.Errorf("shipping.methodRepository.Update: %w", err)
	}
	return method, nil
}

func (r *methodRepository) Delete(ctx context.Context, id int64) error {
	const q = `DELETE FROM shipping_methods WHERE id = $1`

	res, err := r.db.Exec(ctx, q, id)
	if err != nil {
		return fmt.Errorf("shipping.methodRepository.Delete: %w", err)
	}
...
func scanShippingMethod(row pgx.Row) (*ShippingMethod, error) {
