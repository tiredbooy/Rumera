...
func normalizeCreateShippingZone(req *CreateShippingZoneReq) error {
...
func normalizeUpdateShippingZone(req *UpdateShippingZoneReq) error {
	if req.Name.Set {
...
func normalizeCreateShippingMethod(req *CreateShippingMethodReq) error {
	req.Name = strings.TrimSpace(req.Name)
	if req.Name == "" || utf8.RuneCountInString(req.Name) > 100 {
		return apperr.ErrInvalidRequest
	}
...
func normalizeUpdateShippingMethod(req *UpdateShippingMethodReq, current *ShippingMethod) error {
	if req.Name.Set {
		if req.Name.Value == nil {
			return apperr.ErrInvalidRequest
		}
...
func validateShippingMethodDefinition(
	...
	if rateType == ShippingRatePercentage && baseRate > 100 {
		return apperr.ErrInvalidRequest
	}
	if rateType == ShippingRateFree && (baseRate != 0 || freeAbove != nil) {
		return apperr.ErrInvalidRequest
	}
...
func validShippingRateType(rateType ShippingRateType) bool {
	switch rateType {
	case ShippingRateFlat, ShippingRatePerKg, ShippingRatePercentage, ShippingRateFree:
		return true
	default:
		return false
	}
