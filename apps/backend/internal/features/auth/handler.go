...
// Handler is the HTTP + token surface for authentication.
type Handler struct {
...
// LoyaltyAwarder is the optional signup-bonus hook.
type LoyaltyAwarder interface {
	AwardSignup(ctx context.Context, userID int64) error
}

// New constructs the auth feature handler.
...
func (h *Handler) Register(c *gin.Context) {
	...
	req := users.MapToCreateUserReq(input)
	...
	user, err := h.Users.Create(c.Request.Context(), req, hash)
	if err != nil {
...
func (h *Handler) Refresh(c *gin.Context) {
	...
	user, err := h.Users.GetByIDIncludingInactive(c.Request.Context(), userUUID)
	if err != nil {
		response.Error(c, response.ErrInvalidToken)
		return
	}
