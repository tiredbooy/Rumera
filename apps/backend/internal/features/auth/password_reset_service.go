...
func (s *PasswordResetService) RequestReset(ctx context.Context, email string) error {
	...
	async.GoCtx("auth.password_reset_email", 15*time.Second, func(sendCtx context.Context) {
		if s.notifier != nil {
...
func (s *PasswordResetService) ValidateToken(ctx context.Context, token string) (*PasswordReset, error) {
	if token == "" {
...
func assertTokenValid(reset *PasswordReset) error {
	if reset.UsedAt != nil {
		return apperr.ErrInvalidToken
	}
