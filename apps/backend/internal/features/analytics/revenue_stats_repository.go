...
type DailyRevenueStatsRepository interface {
	Upsert(ctx context.Context, req *DailyRevenueStatsUpsertReq) error
	GetByDate(ctx context.Context, date time.Time) (*DailyRevenueStats, error)
	GetRange(ctx context.Context, filter RevenueStatsFilter) ([]*DailyRevenueStats, error)
	Summary(ctx context.Context, filter RevenueStatsFilter) (*RevenueStatsSummary, error)
}
...
func (r *dailyRevenueStatsRepository) Upsert(ctx context.Context, req *DailyRevenueStatsUpsertReq) error {
...
func (r *dailyRevenueStatsRepository) GetByDate(ctx context.Context, date time.Time) (*DailyRevenueStats, error) {
	query := `
		SELECT date,
		       orders_total, orders_completed, orders_cancelled, orders_refunded,
		       gross_revenue, refunds_total, discounts_total, net_revenue, shipping_revenue,
...
func (r *dailyRevenueStatsRepository) GetRange(ctx context.Context, filter RevenueStatsFilter) ([]*DailyRevenueStats, error) {
	query := `
		SELECT date,
		       orders_total, orders_completed, orders_cancelled, orders_refunded,
		       gross_revenue, refunds_total, discounts_total, net_revenue, shipping_revenue,
	...
	stats := make([]*DailyRevenueStats, 0)
	for rows.Next() {
...
func (r *dailyRevenueStatsRepository) Summary(ctx context.Context, filter RevenueStatsFilter) (*RevenueStatsSummary, error) {
	query := `
		SELECT
...
func scanRevenueStats(row pgx.Row) (*DailyRevenueStats, error) {
	s := &DailyRevenueStats{}
	...
	if s.TopCategories == nil {
		s.TopCategories = make([]TopCategoryEntry, 0)
	}
	if s.TopProducts == nil {
		s.TopProducts = make([]TopProductRevenueEntry, 0)
	}
