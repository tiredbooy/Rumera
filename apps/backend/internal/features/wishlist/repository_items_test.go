...
func TestCollectVariantIDs(t *testing.T) {
	t.Run("unique in first-seen order", func(t *testing.T) {
		got := collectVariantIDs([]ItemResponse{
			{VariantID: 3},
			{VariantID: 1},
			{VariantID: 3},
			{VariantID: 0},
	...
	t.Run("empty items", func(t *testing.T) {
		...
		if got := collectVariantIDs([]ItemResponse{}); got != nil {
...
func TestAssignVariantOptions(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			items: []ItemResponse{{VariantID: 7}, {VariantID: 8}},
			options: map[int64][]models.OptionValueResponse{
				7: {colorRed},
				8: {sizeM},
			},
		...
		},
		{
			...
			items: []ItemResponse{{VariantID: 7}, {VariantID: 9}},
			options: map[int64][]models.OptionValueResponse{
				7: {colorRed},
			},
		...
		},
		{
			...
			items: []ItemResponse{{ID: 1, VariantID: 7}, {ID: 2, VariantID: 7}},
			options: map[int64][]models.OptionValueResponse{
				7: {colorRed, sizeM},
			},
		...
		},
		{
			...
			items:   []ItemResponse{{VariantID: 7}},
			options: nil,
