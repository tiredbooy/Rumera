...
type Repository interface {
	Create(ctx context.Context, a ProductAlert) (*ProductAlert, error)
	ListByUser(ctx context.Context, userID int64) ([]ProductAlert, error)
	Delete(ctx context.Context, userID, id int64) error
	// FindPending returns un-notified alerts whose condition is now satisfied.
	FindPending(ctx context.Context, limit int) ([]PendingAlert, error)
...
func (r *alertRepository) Create(ctx context.Context, a ProductAlert) (*ProductAlert, error) {
	const q = `
...
func (r *alertRepository) ListByUser(ctx context.Context, userID int64) ([]ProductAlert, error) {
	...
	alerts, err := pgx.CollectRows(rows, pgx.RowToStructByName[ProductAlert])
	if err != nil {
...
func (r *alertRepository) FindPending(ctx context.Context, limit int) ([]PendingAlert, error) {
	const q = `
		SELECT a.id,
		       u.email                       AS email,
	...
	pending, err := pgx.CollectRows(rows, pgx.RowToStructByName[PendingAlert])
	if err != nil {
