...
func (r *repository) Create(ctx context.Context, userID int64, req CreateAddressReq) (*Address, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("repository.Create insert: %w", err)
	}
...
func (r *repository) GetByID(ctx context.Context, id int64, userID int64) (*Address, error) {
...
func (r *repository) GetAllByUserID(ctx context.Context, userID int64) ([]*Address, error) {
	const q = `
	...
	if err != nil {
		return nil, fmt.Errorf("repository.GetAllByUserID: %w", err)
	}
	defer rows.Close()

	addresses, err := pgx.CollectRows(rows, pgx.RowToStructByName[Address])
	if err != nil {
		return nil, fmt.Errorf("repository.GetAllByUserID scan: %w", err)
	}
...
func (r *repository) Update(ctx context.Context, id int64, userID int64, req UpdateAddressReq) (*Address, error) {
	sets := []string{}
	args := pgx.NamedArgs{
		"id":      id,
		"user_id": userID,
...
func (r *repository) updateWithDefaultTx(ctx context.Context, id int64, userID int64, sets []string, args pgx.NamedArgs) (*Address, error) {
	tx, err := r.db.Begin(ctx)
	if err != nil {
		return nil, fmt.Errorf("repository.Update begin tx: %w", err)
	}
	defer tx.Rollback(ctx)
	...
	rows, err := tx.Query(ctx, q, args)
	if err != nil {
		return nil, fmt.Errorf("repository.Update tx: %w", err)
	}
	...
	if err != nil {
		...
		return nil, fmt.Errorf("repository.Update tx scan: %w", err)
	}
...
func (r *repository) SetDefault(ctx context.Context, id int64, userID int64) error {
	tx, err := r.db.Begin(ctx)
	if err != nil {
		return fmt.Errorf("repository.SetDefault begin tx: %w", err)
	}
	defer tx.Rollback(ctx)
	...
	if err != nil {
		return fmt.Errorf("repository.SetDefault: %w", err)
	}
	if res.RowsAffected() == 0 {
		return models.ErrNotFound
	}
...
func unsetDefault(ctx context.Context, tx pgx.Tx, userID int64) error {
	...
	if _, err := tx.Exec(ctx, q, userID); err != nil {
		return fmt.Errorf("repository.unsetDefault: %w", err)
	}
	return nil
}
