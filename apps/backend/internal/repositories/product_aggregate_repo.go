...
func (r *productRepository) FindAggregateOperation(
	ctx context.Context,
	operationID string,
	requestHash string,
) (*models.ProductAggregateWriteResult, error) {
	...
	return &models.ProductAggregateWriteResult{Product: product, Replayed: true}, nil
}

func (r *productRepository) SaveAggregate(
	ctx context.Context,
	productID int64,
	requestHash string,
	...
	if err != nil {
		return nil, fmt.Errorf("productRepository.SaveAggregate begin: %w", err)
	}
	...
	if _, err := tx.Exec(ctx, `DELETE FROM product_tags WHERE product_id = $1`, productID); err != nil {
		return nil, fmt.Errorf("productRepository.SaveAggregate clear tags: %w", err)
	}
	if err := insertProductTagsTx(ctx, tx, productID, req.TagIDs); err != nil {
		...
		return nil, fmt.Errorf("productRepository.SaveAggregate insert tags: %w", err)
	}
	...
	if _, err := tx.Exec(ctx,
		...
		return nil, fmt.Errorf("productRepository.SaveAggregate complete operation: %w", err)
	}
	if err := tx.Commit(ctx); err != nil {
		return nil, fmt.Errorf("productRepository.SaveAggregate commit: %w", err)
	}

	return &models.ProductAggregateWriteResult{
		Product:      product,
...
func lockAggregateProductGraphTx(ctx context.Context, tx pgx.Tx, productID int64) error {
	if err := lockProductVariantSetTx(ctx, tx, productID); err != nil {
		return fmt.Errorf("productRepository.SaveAggregate variant lock: %w", err)
	}
	if _, err := tx.Exec(ctx,
		`SELECT pg_advisory_xact_lock(hashtextextended('product-images:' || ($1::bigint)::text, 0))`,
		productID,
	); err != nil {
		return fmt.Errorf("productRepository.SaveAggregate image lock: %w", err)
	}
	return nil
}
...
func replaceAggregateVariantsTx(
	...
	tx pgx.Tx,
	productID int64,
	requested []models.SaveProductVariantReq,
) ([]string, error) {
	...
	for i, variant := range requested {
		if err := insertVariantOptionsTx(ctx, tx, variantIDs[i], variant.OptionValueIDs, false); err != nil {
...
func replaceAggregateImagesTx(
	...
	tx pgx.Tx,
	productID int64,
	requested []models.SaveProductImageReq,
) ([]string, error) {
	rows, err := tx.Query(ctx, `
...
func getProductByIDTx(ctx context.Context, tx pgx.Tx, productID int64) (*models.Product, error) {
...
func aggregateFieldError(field, message string, err error) error {
	return &models.FieldError{Field: field, Message: message, Err: err}
}
