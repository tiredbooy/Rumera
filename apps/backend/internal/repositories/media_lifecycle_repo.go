...
// MediaLifecycleRepository provides the database side of safe local-media
...
type MediaLifecycleRepository interface {
	...
	LockMediaKeys(ctx context.Context, keys []string) (MediaKeyLock, error)
	TryReconciliationLock(ctx context.Context) (MediaReconciliationLock, bool, error)
}

type MediaReconciliationLock interface {
	Release(ctx context.Context) error
}

type MediaKeyLock interface {
	Release(ctx context.Context) error
}
...
func NewMediaLifecycleRepository(db *pgxpool.Pool) MediaLifecycleRepository {
...
func (r *mediaLifecycleRepository) LockMediaKeys(ctx context.Context, keys []string) (MediaKeyLock, error) {
...
func (r *mediaLifecycleRepository) TryReconciliationLock(ctx context.Context) (MediaReconciliationLock, bool, error) {
	conn, err := r.db.Acquire(ctx)
	if err != nil {
