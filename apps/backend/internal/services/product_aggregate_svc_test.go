...
type aggregateReplayRepository struct {
	...
	result    *models.ProductAggregateWriteResult
	findCalls int
	saveCalls int
}
...
func (r *aggregateReplayRepository) SaveAggregate(
	...
	int64,
	string,
	models.SaveProductAggregateReq,
) (*models.ProductAggregateWriteResult, error) {
	r.saveCalls++
	return nil, nil
}
...
func TestProductAggregateReplayDoesNotDependOnStagedMedia(t *testing.T) {
	want := &models.Product{ID: 17, Title: "Already committed"}
	repo := &aggregateReplayRepository{
		result: &models.ProductAggregateWriteResult{Product: want, Replayed: true},
	}
	key := "uploads/no-longer-present.webp"
	service := NewProductService(repo, nil, nil)

	got, err := service.SaveAggregate(context.Background(), 0, models.SaveProductAggregateReq{
