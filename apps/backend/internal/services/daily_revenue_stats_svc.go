...
type DailyRevenueStatsService struct {
	repo repositories.DailyRevenueStatsRepository
}

func NewDailyRevenueStatsService(repo repositories.DailyRevenueStatsRepository) *DailyRevenueStatsService {
	return &DailyRevenueStatsService{repo: repo}
}

// FlushStats is called by the cron worker after aggregating order data for a day.
func (s *DailyRevenueStatsService) FlushStats(ctx context.Context, req *analyticsmodels.DailyRevenueStatsUpsertReq) error {
...
func (s *DailyRevenueStatsService) GetByDate(ctx context.Context, date time.Time) (*analyticsmodels.DailyRevenueStats, error) {
...
func (s *DailyRevenueStatsService) GetTimeSeries(ctx context.Context, from, to time.Time) ([]*analyticsmodels.DailyRevenueStats, error) {
	stats, err := s.repo.GetRange(ctx, analyticsmodels.RevenueStatsFilter{
		DateFrom: from,
		DateTo:   to,
	})
	if err != nil {
...
func (s *DailyRevenueStatsService) GetSummary(ctx context.Context, from, to time.Time) (*analyticsmodels.RevenueStatsSummary, error) {
	summary, err := s.repo.Summary(ctx, analyticsmodels.RevenueStatsFilter{
		DateFrom: from,
		DateTo:   to,
	})
	if err != nil {
...
func (s *DailyRevenueStatsService) GetToday(ctx context.Context) (*analyticsmodels.DailyRevenueStats, error) {
	today := time.Now().UTC().Truncate(24 * time.Hour)
	return s.GetByDate(ctx, today)
}

// GetDateRange is a convenience wrapper that validates from <= to before querying.
func (s *DailyRevenueStatsService) GetDateRange(ctx context.Context, from, to time.Time) ([]*analyticsmodels.DailyRevenueStats, error) {
	if from.After(to) {
