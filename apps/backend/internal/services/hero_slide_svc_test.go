...
func TestHeroSlideCreateAllowsOnlyInactiveMediaLessDraft(t *testing.T) {
	...
	service := NewHeroSlideService(repo, nil)

	if _, err := service.Create(context.Background(), &models.HeroSlideReq{Title: "Active"}); err == nil {
...
func TestHeroSlideCreateValidatesMergedPublicationFields(t *testing.T) {
	...
	_, err := service.Create(context.Background(), &models.HeroSlideReq{
		Title:    "Draft",
		CTALabel: &label,
		IsActive: &inactive,
		StartsAt: &start,
...
func TestHeroSlideUpdateMapsConcurrentConstraintFailuresToValidation(t *testing.T) {
	repo := &heroSlideRepositoryStub{
		current:   &models.HeroSlide{ID: 12, Title: "Slide", Theme: "dark"},
		updateErr: models.ErrHeroSchedule,
	}
	service := NewHeroSlideService(repo, nil)

	_, err := service.Update(context.Background(), 12, &models.HeroSlideUpdateReq{})
	requireHeroValidationField(t, err, "ends_at")
}
...
func TestHeroSlideUpdateRequiresMediaBeforeActivation(t *testing.T) {
	repo := &heroSlideRepositoryStub{current: &models.HeroSlide{ID: 7, Title: "Draft", Theme: "dark"}}
	service := NewHeroSlideService(repo, nil)
	active := true

	if _, err := service.Update(context.Background(), 7, &models.HeroSlideUpdateReq{IsActive: &active}); err == nil {
...
func TestHeroSlideUpdateClearsOrdinaryNullableFields(t *testing.T) {
	...
	repo := &heroSlideRepositoryStub{current: &models.HeroSlide{
		ID:                8,
		Title:             "Slide",
		Eyebrow:           &value,
		Subtitle:          &value,
	...
	updated, err := service.Update(context.Background(), 8, &models.HeroSlideUpdateReq{
...
func TestHeroSlideUpdateValidatesMergedCTAPair(t *testing.T) {
	...
	_, err := service.Update(context.Background(), 9, &models.HeroSlideUpdateReq{
...
func TestHeroSlideUpdateAllowsClearAndDeactivateTogether(t *testing.T) {
	...
	updated, err := service.Update(context.Background(), 10, &models.HeroSlideUpdateReq{
