...
func TestVariantSKUAndCombinationValidation(t *testing.T) {
	...
	size, err := optionService.CreateType(ctx, models.CreateOptionTypeReq{
	...
	large, err := optionService.CreateValue(ctx, size.ID, models.CreateOptionValueReq{Value: "Large"})
	if err != nil {
		t.Fatalf("create large: %v", err)
	}
	small, err := optionService.CreateValue(ctx, size.ID, models.CreateOptionValueReq{Value: "Small"})
	if err != nil {
	...
	if _, err := service.Create(ctx, productID, models.CreateVariantReq{
	...
	second, err := service.Create(ctx, productID, models.CreateVariantReq{
	...
	if _, err := service.Update(ctx, second.ID, models.UpdateVariantReq{
	...
	cleared, err := service.Update(ctx, second.ID, models.UpdateVariantReq{
	...
	productService := services.NewProductService(repositories.NewProductRepository(testPool), nil, nil)
	if _, err := productService.Create(ctx, models.CreateProductReq{
	...
	for _, sku := range []string{"Concurrent-1", "Concurrent-2"} {
		sku := sku
		go func() {
			variant, createErr := service.Create(ctx, productID, models.CreateVariantReq{
				SKU: &sku, Price: 14,
