...
export function deleteVariant(variantId: number) {
  return adminRequest<void>(`admin/variants/${variantId}`, {
    method: "DELETE",
  });
}
...
export function uploadProductImage(
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.onabort = () =>
      reject(new AdminApiError(0, "ABORTED", "بارگذاری لغو شد"));
...
export function listAdminRecipes(params: ListRecipesParams = {}) {
  ...
  return adminRequest<Paginated<RecipeListItem>>(
...
export function listUsers(params: ListUsersParams = {}) {
  ...
  const query = qs.toString();
  return adminRequest<Paginated<UserListItem>>(
...
export function getRevenueSummary(
  ...
  return adminRequest<RevenueSummary>(
    `admin/analytics/revenue/summary${buildQuery(range)}`,
  );
}
...
export function getRevenueTimeSeries(
  ...
  return adminRequest<DailyRevenueStat[]>(
    `admin/analytics/revenue/timeseries${buildQuery(range)}`,
  );
}
...
export function listOrders(
  ...
  return adminRequest<Paginated<OrderListItem>>(
