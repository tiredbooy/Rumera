...
import { Loader2, Award, Gift, Sparkles, ArrowDownToLine } from "lucide-react"
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
      ...
      <div className="flex flex-col gap-6">
        <div className="cellar-glow border-hairline relative overflow-hidden rounded-3xl px-6 py-7 ring-1 ring-foreground/10">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 font-serif text-4xl text-foil">{faNum(balance)}</p>
            </div>
        ...
        <div className="border-hairline rounded-3xl bg-card p-6 ring-1 ring-foreground/5">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            ...
            />
            <Button onClick={doRedeem} disabled={redeem.isPending || !amount} className="h-11">
      ...
      <div className="border-hairline h-fit rounded-3xl bg-card p-6 ring-1 ring-foreground/5">
        <h2 className="font-serif text-2xl">تاریخچهٔ امتیاز</h2>
        {txs && txs.length > 0 ? (
          <ul className="mt-4 divide-y divide-border/60">
            {txs.map((t: LoyaltyTx, i) => (
              <li key={i} className="flex items-center justify-between gap-3 py-3 text-sm">
                ...
                <span className={cn("font-serif text-lg", t.delta >= 0 ? "text-emerald-500" : "text-muted-foreground")}>
                  {t.delta >= 0 ? "+" : "−"}
          ...
          <p className="mt-4 text-sm text-muted-foreground">هنوز امتیازی ثبت نشده است.</p>
        )}
      </div>
    </div>
  )
