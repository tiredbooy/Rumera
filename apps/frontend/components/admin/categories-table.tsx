...
export function CategoriesTable({ canWrite }: { canWrite: boolean }) {
  ...
  const [pendingDelete, setPendingDelete] = React.useState<CategoryTreeNode | null>(null)
  ...
  const deleteMutation = useMutation({
    ...
    onError: (e) => {
      if (e instanceof AdminApiError && e.code === "HAS_CHILDREN") {
        toast.error("این دسته‌بندی زیرمجموعه دارد و قابل حذف نیست. ابتدا زیرمجموعه‌ها را حذف یا جابه‌جا کنید.")
    ...
    return (
      <div className="border-hairline flex flex-col items-center justify-center gap-4 rounded-2xl bg-card px-6 py-16 text-center ring-1 ring-foreground/[0.04]">
        ...
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">ارتباط با سرور برقرار نشد. دوباره تلاش کنید.</p>
        </div>
    ...
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-end px-1">
        <span className="rounded-md bg-muted/60 px-2 py-1 text-xs font-medium tabular-nums text-muted-foreground">
          ...
          {isFetching ? <Loader2 className="ms-1.5 inline size-3 animate-spin align-[-1px]" /> : null}
        </span>
      </div>
          ...
          <TableBody>
            {rows.map(({ node, depth, childCount }) => (
              <TableRow key={node.id} className="border-border/40">
                <TableCell>
                  ...
                  >
                    {depth > 0 ? (
                      ...
                      <FolderTree className="size-4 shrink-0 text-muted-foreground/70" aria-hidden />
                    )}
                ...
                <TableCell>
                  {childCount > 0 ? (
                    <span className="text-muted-foreground tabular-nums">{faNum(childCount)}</span>
                  ) : (
                    <span className="text-xs text-muted-foreground">—</span>
                  )}
                </TableCell>
                <TableCell className="text-end">
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button variant="ghost" size="icon" aria-label={`عملیات دستهٔ ${node.name}`}>
