...
function trendOf(curr: number, prev: number): { value: string; positive: boolean } | undefined {
...
export function AnalyticsView() {
  ...
  // ── KPI cards ──────────────────────────────────────────────────────────────
  const kpiRevenue = summary ? num(summary.total_net_revenue) : dashboardSummary.revenue30d
  const kpiOrders = summary ? summary.total_orders : dashboardSummary.orders30d
  const kpiCustomers = summary ? summary.unique_customers : dashboardSummary.activeCustomers
  const rawConv = summary ? num(summary.avg_conversion_rate) : NaN
  const convPct = Number.isFinite(rawConv) ? (rawConv <= 1 ? rawConv * 100 : rawConv) : NaN
  ...
  const revTrend = hasReal && prevWindow?.length
    ? trendOf(sumBy(realWindow!, (d) => num(d.net_revenue)), sumBy(prevWindow, (d) => num(d.net_revenue)))
    : undefined
  const ordTrend = hasReal && prevWindow?.length
    ? trendOf(sumBy(realWindow!, (d) => d.orders_total), sumBy(prevWindow, (d) => d.orders_total))
    : undefined
  ...
  const chartSeries = hasReal
    ? realWindow!.map((d) => ({ day: shortDay(d.date), revenue: num(d.net_revenue), orders: d.orders_total }))
    : revenueSeries.slice(-Math.min(days, revenueSeries.length))

  // ── New vs returning (derived from the real window) ──────────────────────────
  const newSum = hasReal ? sumBy(realWindow!, (d) => d.orders_new_customers) : 0
  ...
  const ordersByStatusData = ordersByStatus.map((o) => ({ label: o.label, value: o.value }))
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          ...
          spark={orderSpark}
        />
        <StatCard label="مشتریان یکتا" value={faNum(kpiCustomers)} icon={Users} />
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <ChartCard title="مشتری جدید در برابر بازگشتی" description="درصد سفارش‌ها">
          <DonutChart data={nvrData} centerValue={`٪${faNum(retPct)}`} centerLabel="بازگشتی" />
          <div className="mt-4">
            <DonutLegend data={nvrData} unit="٪" />
          </div>
        </ChartCard>

        <ChartCard title="وضعیت سفارش‌ها" description="تعداد در هر مرحله · نمونه">
          <div className="pt-2">
            <HorizontalBars data={ordersByStatusData} color="oklch(0.62 0.16 250)" />
          </div>
        </ChartCard>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <ChartCard title="پرفروش‌ترین محصولات" description="بر اساس درآمد · نمونه">
          <HorizontalBars
            data={topData}
