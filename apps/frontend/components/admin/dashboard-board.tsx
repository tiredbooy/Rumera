...
type RecentRow = { id: number; created_at: string; total: number; status: OrderStatus }
type LowRow = { key: string; label: string; available: number; status: StockStatus }
...
export function DashboardBoard() {
  ...
  const newTrend = today ? undefined : { value: s.newCustomersTrend, positive: false }
  const aovTrend = today ? undefined : { value: s.avgOrderValueTrend, positive: true }

  const hasSeries = !!series && series.length > 0
  const chartData = hasSeries
    ? series!.map((d) => ({ day: shortDay(d.date), revenue: num(d.net_revenue) }))
    : revenueSeries
  ...
  const recentRows: RecentRow[] = orders
    ? orders.map((o) => ({ id: o.id, created_at: o.created_at, total: o.total_amount, status: o.status }))
    : recentOrders.map((o) => ({
        id: o.number,
        created_at: o.date,
        total: o.total,
    ...
    <>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          ...
          hint="امروز"
        />
        <StatCard label="میانگین سبد" value={formatPrice(aov)} icon={TrendingUp} trend={aovTrend} />
      </div>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-3">
        ...
        <ChartCard title="سفارش‌ها بر اساس وضعیت" description={`${faNum(totalOrders)} سفارش · نمونه`}>
          <DonutChart data={ordersByStatus} centerValue={faNum(totalOrders)} centerLabel="سفارش" />
          <div className="mt-4">
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-3">
        ...
        <div>
          ...
          <div className="border-hairline divide-y divide-border/50 overflow-hidden rounded-2xl bg-card ring-1 ring-foreground/[0.04]">
            {lowRows.length === 0 ? (
              <p className="p-6 text-center text-sm text-muted-foreground">موجودی همهٔ کالاها سالم است.</p>
            ) : (
              lowRows.map((r) => (
                <div
                  key={r.key}
