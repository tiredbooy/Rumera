...
import { AlertCircle, GripVertical, ImagePlus, Loader2, RotateCcw, Star, Trash2 } from "lucide-react"
...
function validate(file: File): string | null {
  if (!ACCEPT.includes(file.type)) return "فرمت پشتیبانی نمی‌شود (JPG/PNG/WebP/AVIF)"
  if (file.size > MAX_MB * 1024 * 1024) return `حجم باید کمتر از ${MAX_MB} مگابایت باشد`
  return null
}

export const ImageUploader = React.forwardRef<
  ImageUploaderHandle,
  { productId?: number | null; initialImages?: AdminProductImage[] }
>(function ImageUploader({ productId, initialImages = [] }, ref) {
  ...
  // In edit mode, hydrate from the live admin image list (which carries storage
  // keys) unless the parent already seeded it. Runs once per product.
  const seededRef = React.useRef(initialImages.length > 0)
  React.useEffect(() => {
  ...
  React.useEffect(() => {
    return () => {
      setSlots((cur) => {
        cur.forEach((s) => s.kind === "staged" && URL.revokeObjectURL(s.previewUrl))
        return cur
      })
    }
  }, [])

  const patch = React.useCallback((localId: string, next: Partial<StagedSlot>) => {
    setSlots((cur) =>
      cur.map((s) => (s.localId === localId && s.kind === "staged" ? { ...s, ...next } : s))
    )
  }, [])

  async function uploadStaged(slot: StagedSlot, pid: number): Promise<AdminProductImage | null> {
      ...
      setSlots((cur) =>
        cur.map((s) =>
          s.localId === slot.localId
            ? { kind: "uploaded", localId: s.localId, image, alt: image.alt_text ?? "" }
            : s
        )
      )
      return image
  ...
  function addFiles(files: FileList | File[]) {
    ...
    setSlots((cur) => {
      const hasPrimary = cur.some((s) => (s.kind === "uploaded" ? s.image.is_primary : s.isPrimary))
      // First-ever image becomes primary by default.
      if (!hasPrimary && incoming[0] && !incoming[0].error) incoming[0].isPrimary = true
      return [...cur, ...incoming]
    })
    if (live) {
      incoming.forEach((s) => {
  ...
  function removeSlot(slot: Slot) {
    ...
    setSlots((cur) => cur.filter((s) => s.localId !== slot.localId))
    if (live) {
      deleteProductImage(productId, slot.image.id).catch(() => {
  ...
  function makePrimary(slot: Slot) {
    setSlots((cur) =>
      cur.map((s) => {
        ...
        if (s.kind === "uploaded") return { ...s, image: { ...s.image, is_primary: isThis } }
        return { ...s, isPrimary: isThis }
      })
    )
    if (live && slot.kind === "uploaded") {
      setPrimaryImage(productId, slot.image.id).catch(() => {})
    }
  }

  function setAlt(slot: Slot, alt: string) {
    setSlots((cur) => cur.map((s) => (s.localId === slot.localId ? { ...s, alt } : s)))
  }

  function commitAlt(slot: Slot) {
    if (live && slot.kind === "uploaded") {
      updateImageAlt(productId, slot.image.id, slot.alt).catch(() => {})
    }
  }
  ...
  function move(from: number, to: number) {
    ...
    setSlots((cur) => {
      ...
      next.splice(to, 0, m)
      if (live) {
        const ids = next.filter((s): s is UploadedSlot => s.kind === "uploaded").map((s) => s.image.id)
        if (ids.length > 1) reorderProductImages(productId, ids).catch(() => {})
      }
      return next
    })
  }
  ...
  React.useImperativeHandle(
    ...
    () => ({
      ...
      flush: async (pid: number) => {
        // Upload staged files in current display order, tracking the resulting
        // id (and which slot is primary) so we can persist both afterwards.
        ...
        if (ordered.length > 1) await reorderProductImages(pid, ordered).catch(() => {})
        if (primaryId !== null) await setPrimaryImage(pid, primaryId).catch(() => {})
      },
    }),
  ...
  const previewOf = (slot: Slot) =>
    slot.kind === "uploaded" ? (
      ...
      <img src={slot.previewUrl} alt="پیش‌نمایش" className="h-full w-full object-cover" />
    )
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      >
        ...
        <span className="text-xs">برای انتخاب یا کشیدن تصاویر اینجا کلیک کنید</span>
        <span className="text-[11px]">۱۰۰۰×۱۲۵۰ پیکسل پیشنهادی · JPG/PNG/WebP/AVIF · حداکثر {MAX_MB}MB</span>
      </button>
      <input
        ref={inputRef}
        type="file"
      ...
      {slots.length > 0 ? (
        <ul className="flex flex-col gap-2">
          {slots.map((slot, i) => {
            const isPrimary = slot.kind === "uploaded" ? slot.image.is_primary : slot.isPrimary
                  ...
                  {uploading ? (
                    <div className="h-1 overflow-hidden rounded-full bg-muted">
                      <div
                        ...
                        style={{ width: `${Math.round((slot.progress || 0) * 100)}%` }}
                      />
                    </div>
                  ) : errored ? (
                    <p className="flex items-center gap-1 text-xs text-destructive">
                      <AlertCircle className="size-3" aria-hidden /> {slot.error}
                    </p>
                  ) : isPrimary ? (
                ...
                <div className="flex items-center gap-0.5">
                  {errored && live ? (
                    <Button
                      ...
                      onClick={() => slot.kind === "staged" && uploadStaged(slot, productId)}
                    >
                  ...
                  <Button
                    ...
                    disabled={uploading || errored}
                    onClick={() => makePrimary(slot)}
                  >
                    <Star className={cn("size-4", isPrimary && "fill-primary text-primary")} />
                  </Button>
                  <Button
                    type="button"
                    variant="ghost"
