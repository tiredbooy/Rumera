...
const STATUS_META: Record<string, { label: string; variant: "secondary" | "outline" | "destructive" }> = {
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0">
        <p className="text-sm font-medium leading-tight">شارژ کیف پول از طریق درگاه</p>
        <p className="mt-0.5 text-xs text-muted-foreground">به‌زودی فعال می‌شود</p>
      </div>
    ...
    <div className="border-hairline rounded-2xl bg-card p-4 ring-1 ring-foreground/5">
      <p className="flex items-center gap-1.5 text-xs text-muted-foreground">
        <Icon
          className={cn(
            "size-3.5",
            tone === "credit" ? "text-emerald-600 dark:text-emerald-400" : "text-muted-foreground"
          )}
          aria-hidden
        />
        {label}
      ...
      {loading ? (
        ...
        <p
          className={cn(
            ...
            tone === "credit" ? "text-emerald-600 dark:text-emerald-400" : "text-foreground"
          )}
          dir="ltr"
        >
          {tone === "credit" ? "+" : "−"}
    ...
    <>
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            {wallet.isLoading ? (
              ...
              <p className="mt-1 font-serif text-5xl text-foil" dir="ltr" data-testid="wallet-balance">
      ...
      >
        ...
        {txs.isLoading ? (
          ...
          <EmptyState
            icon={Wallet}
            title={hasActiveFilter ? "تراکنشی با این فیلتر یافت نشد" : "تراکنشی برای نمایش نیست"}
            description={
              hasActiveFilter
          ...
          <>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-end">موجودی پس از تراکنش</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {pageRows.map((t: WalletTransaction) => {
                    ...
                    const voided = t.status === "failed" || t.status === "cancelled"
                    return (
                      <TableRow key={t.id} className={cn(voided && "opacity-60")}>
                        <TableCell>
                        ...
                        <TableCell
                          ...
                          dir="ltr"
                        >
                          {typeof t.balance_after === "number" ? formatPrice(t.balance_after) : "—"}
                        </TableCell>
                      </TableRow>
                    )
                  })}
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border/60 px-5 py-3 sm:px-6">
              <p className="text-xs text-muted-foreground">
                ...
                {totalPages > 1 ? ` · صفحهٔ ${faNum(safePage)} از ${faNum(totalPages)}` : ""}
              </p>
              {totalPages > 1 ? (
                <nav className="flex items-center gap-1" aria-label="صفحه‌بندی تراکنش‌ها">
                  <Button
                    type="button"
                    variant="outline"
                    size="sm"
