...
import { Star, Loader2, Trash2, MessageSquare, MessageSquarePlus, PencilLine } from "lucide-react"
...
const REVIEW_STATUS_FA: Record<string, { label: string; variant: "secondary" | "default" | "destructive" }> = {
...
function Stars({ value }: { value: number }) {
  return (
    <span className="inline-flex items-center gap-0.5" aria-label={`${faNum(value)} از ۵`}>
...
export function ReviewsView() {
  ...
  return (
    <>
      <Tabs value={tab} onValueChange={setTab}>
        <TabsList className="mb-6">
          ...
          <TabsTrigger value="pending" className="cursor-pointer">
            در انتظار نظر {pendingList.length > 0 ? `(${faNum(pendingList.length)})` : ""}
          </TabsTrigger>
        </TabsList>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-3 flex items-center gap-1">
                        <Button variant="ghost" size="sm" asChild>
                          <Link href={productHref(r.product_slug, r.product_id)}>
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">
                      ...
                      {p.delivered_at ? ` · تحویل ${faDate(p.delivered_at)}` : ""}
                    </p>
                  </div>
      ...
      <AlertDialog open={toDelete !== null} onOpenChange={(o) => (o ? null : setToDelete(null))}>
        <AlertDialogContent>
          <AlertDialogHeader>
