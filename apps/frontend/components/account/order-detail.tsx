        ...
        toast.success("اقلام سفارش به سبد خرید افزوده شد", {
          description:
            res.skipped.length > 0
              ...
              : undefined,
          action: { label: "مشاهدهٔ سبد", onClick: () => (window.location.href = "/cart") },
        })
      },
        ...
        <AccountSection title="اقلام سفارش">
          <ul className="divide-y divide-border/60">
            {order.items.map((item) => (
              <li key={item.id} className="flex items-center gap-4 py-4 first:pt-0 last:pb-0">
                ...
                <span className="shrink-0 font-medium">{formatPrice(item.total_price)}</span>
              </li>
            ))}
          </ul>
          ...
          <dl className="mt-5 space-y-2 border-t border-border/60 pt-5 text-sm">
            ...
            {order.discount_amount > 0 ? (
              <Row label="تخفیف" value={`− ${formatPrice(order.discount_amount)}`} />
            ) : null}
            {order.tax_amount > 0 ? <Row label="مالیات" value={formatPrice(order.tax_amount)} /> : null}
            <Row
              label="ارسال"
              value={order.shipping_cost > 0 ? formatPrice(order.shipping_cost) : "رایگان"}
            />
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="text-sm leading-relaxed">
                ...
                <p className="text-muted-foreground">
                  {shipping.state_province ? `${shipping.state_province}، ` : ""}
      ...
      <div className="flex flex-col gap-5">
        <AccountSection title="وضعیت سفارش">
          <div className="mb-4 flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground">{faDate(order.created_at)}</span>
          </div>
          <OrderStatusStepper status={order.status} orientation="vertical" />
          <p className="mt-2 text-xs text-muted-foreground">
            روش پرداخت: {PAYMENT_FA[order.payment_method] ?? order.payment_method}
          </p>
        </AccountSection>
