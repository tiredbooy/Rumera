...
**Also read:** [Domain map](../features/domain-map.md) (where each feature lives) ·
[System overview](../../../../docs/SYSTEM-OVERVIEW.md) (full monorepo).
...
(the `pages.signIn` route) and must be kept in lock-step with `proxy.ts`,
...
[`proxy.ts`](../../proxy.ts) is the **coarse, edge-runtime** access gate. It
...
**Defense in depth:** the edge proxy is intentionally _not_ the authority. The
...
- [`auth.config.ts`](../../lib/auth/auth.config.ts) — Edge-safe slice (page routes +
  ...
  onto the session). No Node-only code, so the edge proxy can use it.
...
Protected page proxy sends expiring sessions through
