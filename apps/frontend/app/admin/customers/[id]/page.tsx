...
import { adminCustomers, ordersForCustomer, TIER_LABELS } from "@/lib/admin/data"
  ...
  return (
    <>
        ...
        <div className="flex flex-wrap items-center gap-x-6 gap-y-4">
          ...
          <dl className="grid flex-1 grid-cols-2 gap-x-6 gap-y-4 text-sm sm:grid-cols-4">
            ...
            <div className="min-w-0">
              ...
              <dd className="mt-1 truncate font-medium" dir="ltr">{user.email}</dd>
            </div>
            <div>
              <dt className="text-xs text-muted-foreground">نقش</dt>
              <dd className="mt-1"><UserRoleBadge role={user.role} /></dd>
            </div>
            <div>
              <dt className="text-xs text-muted-foreground">وضعیت</dt>
              <dd className="mt-1"><UserStatusBadge active={user.is_active} /></dd>
            </div>
            {user.phone ? (
              <div className="min-w-0">
                <dt className="text-xs text-muted-foreground">تلفن</dt>
                <dd className="mt-1 font-medium tabular-nums" dir="ltr">{user.phone}</dd>
              </div>
            ) : null}
            <div>
              <dt className="text-xs text-muted-foreground">تاریخ عضویت</dt>
              <dd className="mt-1 font-medium tabular-nums">{faDate(user.created_at)}</dd>
            </div>
          </dl>
        </div>
      </div>
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="ارزش کل" value={formatPrice(sample.ltv)} icon={Coins} />
        <StatCard label="سفارش‌ها" value={faNum(sample.ordersCount)} icon={ShoppingBag} />
        <StatCard label="میانگین سبد" value={formatPrice(sampleAvgOrder)} icon={TrendingUp} />
        <StatCard label="کیف پول" value={formatPrice(sample.walletBalance)} icon={Wallet} />
      </div>
        ...
        <div className="border-hairline h-fit rounded-2xl bg-card p-5 ring-1 ring-foreground/[0.04]">
          ...
          <dl className="space-y-3 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <dd className="font-medium">{formatPrice(sample.walletBalance)}</dd>
            </div>
            <div className="flex justify-between">
